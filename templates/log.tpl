...
{block name=main}
 ...
 {foreach from=$revlist item=rev}
   <div class="title">
     <a href="{geturl project=$project action=commit hash=$rev}" class="title"><span class="age">{agestring age=$rev->GetAge()|escape}</span>{$rev->GetTitle()|escape:'html'}</a>
   ...
   <div class="title_text">
     ...
     <em>{$rev->GetAuthorName()|escape} [<time datetime="{$rev->GetAuthorEpoch()|date_format:"%Y-%m-%dT%H:%M:%S+00:00"}">{$rev->GetAuthorEpoch()|date_format:"%a, %d %b %Y %H:%M:%S %z"}</time>]</em><br />
   </div>
   <div class="log_body">
   ...
   <div class="page_body">
     {if $commit}
       {capture name=commitage assign=commitage}
         <time datetime="{$commit->GetCommitterEpoch()|date_format:"%Y-%m-%dT%H:%M:%S+00:00"}">{agestring age=$commit->GetAge()|escape}</time>
       {/capture}
